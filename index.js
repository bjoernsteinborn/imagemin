		...
		return pipe
			.then(buf => {
				...
				buf =
					...
					(fileType(buf) && fileType(buf).ext === "webp")
						? buf
						: data;
...
module.exports = (input, output, opts) => {
	...
	return globby(input, { onlyFiles: true }).then(paths =>
		Promise.all(paths.map(x => handleFile(x, output, opts)))
	);
};
...
module.exports.buffer = (input, opts) => {
	...
	return pPipe(opts.plugins)(input).then(
		buf => (buf.length < input.length ? buf : input)
	);
};
